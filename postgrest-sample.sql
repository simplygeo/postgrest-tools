...
-- 3. authenticated user: __authenticated_user__
...
-- authenticated user
drop role if exists __authenticated_user__;
create role __authenticated_user__ nologin;
grant __authenticated_user__ to authenticator;

grant usage on schema api to __authenticated_user__;
grant all on api.todos to __authenticated_user__;
grant usage, select on sequence api.todos_id_seq to __authenticated_user__;
...
insert into basic_auth.users(email, pass, role)
    values('__authenticated_user__@gmail.com', '__authenticated_user__', 		 		
    '__authenticated_user__');
